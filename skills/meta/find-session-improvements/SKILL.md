...
**An orchestrator.** `jankolenko-skills:improve-skill`, `jankolenko-skills:find-skill-gaps`
and `jankolenko-skills:record-engineering-rule` each own a destination and its gate; this
skill decides only what gets looked at. In the long session worth a retrospective,
...
```bash
"${CLAUDE_SKILL_DIR}/../../../scripts/session-spine.sh" <session-id>   # get_session("self") gives the id
```
...
instructs → `jankolenko-skills:improve-skill`; a capability nothing covers → one dated line
in `observations/SIGNALS.md`; how code is written → `jankolenko-skills:record-engineering-rule`;
how one repo builds or runs → out of scope, name it and stop. A lead survives only if it
...
invocation. Each owner applies its own gate, shows its own diff and ships through
`scripts/ship.sh`. Never edit their files yourself: the moment this skill writes directly,
...
- It never edits a skill, a rule or the ledger, and never touches the work repo.
