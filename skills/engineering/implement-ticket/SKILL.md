...
| 3 | `jankolenko-skills:plan-change` | `ticket.*` → `goal`/`criteria`/`candidates`/`constraints`; → `plan.*`; 🛑 verdict |
...
| 6 | inline + `jankolenko-skills:write-tests` | `plan.steps`/`plan.lanes`; `ticket.acceptance_criteria` → `criteria` |
| 7 | `jankolenko-skills:write-tests` | → `tests.result`; 🛑 three failed attempts → stop with the output; never weaken a test |
| 8a | `jankolenko-skills:critique-plan` | `plan.*` (in full: it runs in its own context) + diff → `critique.verdict` |
...
round of decisions, so bail out with `plan.open_questions`; `no-change-needed` → no code
change, no ticket change, report the evidence; `ready` → continue.
...
## Step 8 — Critique, then review

The critique runs first, because polishing code that should not exist is waste.
`reject-to-plan` → Step 3 with what the code revealed, branch and ticket untouched;
`reject-to-code` → Step 6, then 7, then critique again; `accept` → review. A second
